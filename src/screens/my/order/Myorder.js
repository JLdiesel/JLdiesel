...
class orders extends PureComponent {
  ...
  addOrder = () => {
    ...
    if (this.state.addressId !== -1 && this.state.activeTab !== -1) {
      const { count, shopId, id,color,title,price,img } = this.props.route.params;
      const data = {
        shopId,
        shopCarId: id,
        count,
      ...
      addOrider(data).then((res) => this.context.navigate('paySuccess',{...data,address:this.state.defaultAddress}));
    } else {
    ...
    return (
      <View style={{ backgroundColor: '#e2f4fe', flex: 1 }}>
        ...
        <ScrollView style={{ height: pxToDp(675) }}>
          ...
          {/* 收获地址 */}
          <Address changeAddressId={this.changeAddressId}  changeDefaultAddress={this.changeDefaultAddress}   />
          {/* 商品信息 */}
          <TouchableOpacity
            style={{
              height: pxToDp(140),
