...
{   this.props.collect.map((item,collectid)=>(
    <View key={collectid} style={{elevation:3,borderWidth:0,flex:1,width:'95%',alignSelf:'center'}}>
          
        <View style={{flexDirection:'row',marginTop:pxToDp(10),}}>
            <Image source={{uri:item.collectimg}}style={{width:pxToDp(100),height:pxToDp(80),borderRadius:5,top:pxToDp(-3)}}/> 
            <Text style={{width:pxToDp(245),paddingLeft:pxToDp(10)}} numberOfLines={2}>{item.title}</Text>
            </View>
            <View style={{flexDirection:'row',justifyContent:'space-around'}}> 
            <SvgUri svgXmlData={playnumber} width='15' height='15' style={{position:'absolute', bottom:pxToDp(10),left:pxToDp(110)}}/>
            <Text style={{position:'absolute', bottom:pxToDp(10),left:pxToDp(130),fontSize:pxToDp(10)}}>{item.playnum}</Text>
            <SvgUri svgXmlData={playnumber} width='15' height='15' style={{position:'absolute', bottom:pxToDp(10),right:pxToDp(120)}}/>
            <Text style={{position:'absolute', bottom:pxToDp(10),right:pxToDp(100),fontSize:pxToDp(10)}}>{item.playnum}</Text>
            <TouchableOpacity>
